...
class _CharacteristicTileState extends State<CharacteristicTile> {
  ...
  Future onWritePressed() async {
    ...
    return showDialog(
      ...
      builder: (context) {
      return AlertDialog(
        title: const Text('Write to Controller'),
        content: TextField(
        ...
        actions: <Widget>[
          ...
          TextButton(
            ...
            onPressed: () async {
              List<int> value = controller.text.split(",").map((e) => int.parse(e)).toList();
              try {
  ...
  List<int> countUp() {
    writeValues = writeValues.map((e) => (e + 1) % 256).toList();
    return writeValues;
  }

  // Future onWritePressed() async {
  //   try {
  //     await c.write(countUp(), withoutResponse: c.properties.writeWithoutResponse);
  //     Snackbar.show(ABC.c, "Write: Success", success: true);
  //     if (c.properties.read) {
  //       await c.read();
  //     }
